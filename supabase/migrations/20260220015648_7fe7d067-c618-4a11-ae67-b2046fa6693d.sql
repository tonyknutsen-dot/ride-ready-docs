...
-- Add snapshot columns for completion accountability
ALTER TABLE public.compliance_events
  ADD COLUMN IF NOT EXISTS completed_by_name text,
  ADD COLUMN IF NOT EXISTS completed_by_role text;

-- Update complete_event RPC to accept and store snapshot fields
CREATE OR REPLACE FUNCTION public.complete_event(
  ...
  p_completed_by_role text DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET status = 'completed',
      ...
      completed_by_role = p_completed_by_role,
