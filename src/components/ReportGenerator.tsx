  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const overdueInspections = inspections?.filter(i => 
        i.status === 'pending' && i.check_date < format(new Date(), 'yyyy-MM-dd')
      ).length || 0;
...
Period: ${format(start, 'PPP')} to ${format(end, 'PPP')}
...
Generated by Ride Ready Docs
    ...
    a.href = url;
    a.download = `compliance-report-${format(new Date(), 'yyyy-MM-dd')}.${format === 'pdf' ? 'txt' : 'csv'}`;
